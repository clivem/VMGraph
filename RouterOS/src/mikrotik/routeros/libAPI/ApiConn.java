...
public class ApiConn extends Thread {
	...
	private static final Logger logger = Logger.getLogger(ApiConn.class);
	...
	LinkedBlockingQueue<Response> queue = new LinkedBlockingQueue<Response>(40);
	 ...
	 * @param ipPort
	 *            - port used for connection, ROS default is 8728
	 */
	 ...
	 * 
	 * @param s
	 *            - command will be sent to RouterOS for example
	 *            "/ip/address/print\n=follow="
	 * @return
	 */
	...
	/**
	 * Tries to fetch data that is replied to commands sent. It will wait till
	 * it can return something.
	 * 
	 * @return returns data sent by RouterOS
	 * @throws java.lang.InterruptedException
	 */
	public Response getData() throws InterruptedException {
	 ...
	 * 
	 * @param name
	 *            - username of the user on the router
	 * @param password
	 *            - password for the user
	 * @return
	 */
	public boolean login(String name, char[] password) {
		int success = this.sendCommand("/login");
		if (success > -1) {
			String s = "";
			try {
				s = this.getData().getText();
			} catch (InterruptedException ex) {
				//Logger.getLogger(ApiConn.class.getName()).log(Level.SEVERE, null, ex);
				logger.log(Level.WARN, "failed read #1", ex);
				//return "failed read #1";
				//System.out.println("failed read #1");
				return false;
			}
			if (!s.contains("!trap") && s.length() > 4) {
				...
				if (tmp.length > 1) {
					...
					String chal = "";
					chal = Hasher.hexStrToStr("00") + new String(password)
							+ Hasher.hexStrToStr(transition);
					chal = Hasher.hashMD5(chal);
					String m = "/login\n=name=" + name + "\n=response=00" + chal;
					if (this.sendCommand(m) > -1) {
						try {
							s = this.getData().getText();
						} catch (InterruptedException ex) {
							//Logger.getLogger(ApiConn.class.getName()).log(Level.SEVERE, null, ex);
							logger.log(Level.WARN, "failed read #2", ex);
							//return "failed read #2";
							//System.out.println("failed read #2");
							return false;
						}
						if (s.contains("!done")) {
							if (!s.contains("!trap")) {
		...
		logger.log(Level.WARN, "Login failed!");
		return false;
	}
