...
public class WriteCommand {
	...
	private byte[] writeLen(String command) {
		...
		if (s.length() < 2) {
			...
			for (int j = 0; j < s.length(); j += 2) {
				ret += (char) Integer.parseInt(s.substring(j, j + 2), 16) != 0 ? 
						(char) Integer.parseInt(s.substring(j, j + 2), 16) : "";
			}
		}
	...
	int runCommand() {
		...
		try {
			...
			out.write(ret);
			//System.out.println("Sent successfully: [" + command + "]");
			logger.log(Level.INFO, "Sent successfully: [" + command + "]");
			return tag;
			//return "Sent successfully";
		} catch (IOException ex) {
			//Logger.getLogger(WriteCommand.class.getName()).log(Level.SEVERE, null, ex);
			logger.log(Level.WARN, "Error sending command: [" + command + "]", ex);
			//return "failed";
			return -1;
		}
	}
